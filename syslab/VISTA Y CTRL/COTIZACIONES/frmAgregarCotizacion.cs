...
{
    public partial class frmAgregarCotizacion : Form
    {
        long _ID_COTIZACION;
        ...
        public frmAgregarCotizacion(long ID, bool visor = false)
        {
            InitializeComponent();

            this._EDITANDO = true;
            this.lblTitulo.Text = this.Text = "Editando Cotización";

            this._ID_COTIZACION = ID;

            var ps = new CTRL.Persistencia();
            ps.AbrirConexion();

            var reg = ps.SelPorLlave<COTIZACION>(ID.ToString());
            ...
            if (reg != null)
            {
                ...
                foreach (var item in reg.COTIZACION_DETALLEs)
                {
                ...
                if (visor)
                {
                    ...
                    txtNombre.ReadOnly = txtNRC.ReadOnly = txtNIT.ReadOnly = txtGiro.ReadOnly = true;
                }
            ...
            {
                MessageBox.Show("El registro no fue encontrado.", "SysLab", MessageBoxButtons.OK, MessageBoxIcon.Information);
                this.Dispose();
            }
        ...
        private void cmdGuardar_Click(object sender, EventArgs e)
        {
            ...
            { //GENERANDO DETALLE 
                List<COTIZACION_DETALLE> Detalle = new List<COTIZACION_DETALLE>();
                foreach (DataGridViewRow item in dgMovs.Rows)
                {
                    if (!item.IsNewRow)
                    {
                        COTIZACION_DETALLE reg = new COTIZACION_DETALLE();
                ...
                {//AGREGANDO 
                    ...
                    if (CTRL.Examenes.Cotizaciones.AgregarCotizacion(ps.DB, long.Parse(txtPacienteID.Text), long.Parse(txtDoctorID.Text),
                        dtpFecha.Value, dtpRequerido.Value, txtNombre.Text.Trim(), txtGiro.Text.Trim(),
                        ...
                        )
                    {
                        ...
                        MessageBox.Show("La cotización fue agregada satisfactoriamente.", "SysLab", MessageBoxButtons.OK,
                    ...
                    else
                    {
                        ...
                        MessageBox.Show("El registro no pudo ser agregado satisfactoriamente, por favor verifique que la información sea correcta.\n");
                    }
                }
                else
                {//EDITANDO 
                    ...
                    if (CTRL.Examenes.Cotizaciones.EditarCotizacion(ps.DB, this._ID_COTIZACION, long.Parse(txtPacienteID.Text),
                        long.Parse(txtDoctorID.Text), dtpFecha.Value, dtpRequerido.Value, txtNombre.Text.Trim(), txtGiro.Text.Trim(), 
                        txtNIT.Text.Trim(), txtNRC.Text.Trim(), ckRetencion.Checked, ckCFiscal.Checked, this.ptjIVA, this.ptjReten, 
                        this.gravado, this.iva, this.subtotal, this.exento, this.retencion, this.total, Detalle)
                        )
                    {
                    ...
                    {
                        ps.RevertirTransaccion();
                        MessageBox.Show("El registro no pudo ser procesado satisfactoriamente, por favor verifique que la información sea correcta.\n");
                    }
                }
            }
        }
